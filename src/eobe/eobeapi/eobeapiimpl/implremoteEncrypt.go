...
func newAPIRemoteEncrypt(apiParamInput string) (ApiInf, error) {
	...
	return &api, api.parseParameter(apiParamInput, api.paramCount, cStrRemoteEncrypt)
}

func (api *remoteEncrypt) RunAPI(qryKVMap map[string]string, preCallRslts map[string]string) (map[string]string, *APIError) {
	...
	if !found {
		return nil, NewAPIErrorf(CErrBadCallError, cStrParameterNotFound, api.apiValueVarInput[3])
	}
	...
	if len(resStr) == 0 || err != nil {
		return nil, NewAPIErrorf(CErrServerInternalError, cStrFetchRespError, err)
	}
